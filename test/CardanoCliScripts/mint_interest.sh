...
echo "minting interest MONY coins"
...
output=2000000
tokenamount=50
tokenname=$(echo -n "MONY" | xxd -b -ps -c 80 | tr -d '\n')

tokenschange=$(cardano-cli-balance-fixer change --address $(cat ../borrower/payment.addr) ${MAGIC})

cardano-cli transaction build \
	${MAGIC} \
	${SOURCE} \
	--tx-in-collateral $collat \
	--tx-out $address+$output+"$tokenschange" \
